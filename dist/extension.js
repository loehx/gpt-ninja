...
const openai_1 = __webpack_require__(3);
exports["default"] = async () => {
    ...
    if (editor) {
        ...
        if (!apiKey) {
            vscode.window.showErrorMessage("Please set the API key in the extension settings.");
            return;
        }
        vscode.window.showInformationMessage("Asking ChatGPT ...");
        try {
            const chatGPTResponse = await (0, openai_1.sendToChatGPT)(selectedText, rules, apiKey);
...
async function generateSummary(text, rules, apiKey) {
    try {
        ...
        console.error(error);
        throw new Error("Something went wrong while generating the summary. Please try again later.");
    }
}
/**
 * Transforms a chat response from GPT to JavaScript code
 */
function transformChatGPTResponseToJS(chatGPTResponse) {
...
const vscode = __webpack_require__(1);
const sendSelectionToChatGPT_1 = __webpack_require__(2);
function activate(context) {
    console.log('Congratulations, your extension "gpt-ninja" is now active!');
    let helloWorldDisposable = vscode.commands.registerCommand("gpt-ninja.helloWorld", () => {
        vscode.window.showInformationMessage("Hello World from gpt-ninja!");
    });
    let sendSelectionDisposable = vscode.commands.registerCommand("gpt-ninja.sendSelectionToChatGPT", sendSelectionToChatGPT_1.default);
    // Check if API key is set, otherwise open settings
    const config = vscode.workspace.getConfiguration("gpt-ninja");
    const apiKey = config.get("apiKey", "");
    if (!apiKey) {
        vscode.commands.executeCommand("workbench.action.openSettings", "gpt-ninja.apiKey");
    }
